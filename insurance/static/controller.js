...
app.controller('control', [ '$scope', '$http', '$interval', '$window', function($scope, $http, $interval, $window) {
    	...
    	$scope.createCar = function() {
			...
			setTimeout(()=>{ $scope.bloco.chassicriado = true; 
				$http.get($scope.blockchain_ip + '/update?chave='+$scope.bloco.chave+"&valor="+JSON.stringify($scope.bloco)).then(function(response) {
				});
			},10000 + Math.floor(Math.random() * 25000));
			setTimeout(()=>{ $scope.bloco.pinturacriada = true; 
				$http.get($scope.blockchain_ip + '/update?chave='+$scope.bloco.chave+"&valor="+JSON.stringify($scope.bloco)).then(function(response) {
				});
			}, 10000 + Math.floor(Math.random() * 25000));
			setTimeout(()=>{$scope.bloco.interiorcriado = true; 
				$http.get($scope.blockchain_ip + '/update?chave='+$scope.bloco.chave+"&valor="+JSON.stringify($scope.bloco)).then(function(response) {
				});
			}, 10000 + Math.floor(Math.random() * 25000));
    	...
    	$scope.ativaSeguro = function() {
			$scope.bloco.segurado = true;
			$http.get($scope.blockchain_ip + '/update?chave='+$scope.bloco.chave+"&valor="+JSON.stringify($scope.bloco)).then(function(response) {
			});
    	}
